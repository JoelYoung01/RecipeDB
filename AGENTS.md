...
- Backend: FastAPI app in `api/` (entry `api/main.py`), SQLite database (no external DB needed), migrations via Alembic.
- Frontend: Vue 3 + Vite + Vuetify SPA in `src/` (dev server on port 5173).

Standard commands live in `README.md` and `package.json`. Below are only the non-obvious caveats.
...
- Non-obvious: the FastAPI settings (`api/core/config.py`) point `env_file` at `../.env` (outside the repo when run from the root), so the backend effectively relies on process env vars. Export `.env` the same way before running the backend: `source venv/bin/activate && set -a && . ./.env && set +a && fastapi dev api/main.py`.

### Database setup (not part of the dependency update script)

- The SQLite DB lives at `data/database.db`. Create dirs and run migrations + seed once: `mkdir -p data/uploads data/logs && alembic upgrade head && python -m api.scripts.load_data`. The seed creates one admin user (from `SUPERUSER_GID`) and 10 sample recipes. This is state setup, so it is intentionally NOT in the startup update script.

### Running the services

- Backend: `source venv/bin/activate && set -a && . ./.env && set +a && fastapi dev api/main.py --host 0.0.0.0 --port 8000` (docs at `/api/docs`).
...
- Login is Google OAuth only (`/auth/login-google/`), so there is no username/password login. To exercise authenticated UI/endpoints without real Google credentials, mint a session token for the seeded admin user using the app's own logic and inject it into browser `localStorage` under key `access_token`:
  ```python
  # run with venv active and .env exported
...
- Python linters `flake8` and `black` are installed but have pre-existing findings in `api/core/seed_database.py` (long seed strings) and the Alembic migrations; these are not from new work.
